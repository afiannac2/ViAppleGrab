...
namespace ViToolkit
{
    namespace PSMoveSharp
    {
        ...
        public class PSMoveSharpCameraFrameStateCollector
        {
            ...
            public void CaptureState(int index, PSMoveSharpState state)
            {
                if (index == current_index_)
                {
                    ...
                    if (IsSetComplete())
                    {
                        ...
                        Debug.WriteLine("PsMoveSharp Message: Complete set packet index {0}", complete_state_.packet_index);
            ...
            public System.Drawing.Image MakeCompleteImage()
            {
                System.Drawing.Image full_frame = new Bitmap(ImageWidth, ImageHeight);
        ...
        public class PSMoveSharpCameraFrameState
        {
            ...
            protected System.Drawing.Image full_image;
            public const int ImageWidth = 640;
            public const int ImageHeight = 480;
            ...
            public void ProcessPacket(int index, System.Drawing.Image image, int row_start, int row_count)
            {
        ...
        public class PSMoveClient
        {
            ...
            {
                //Test the ability to connect before trying to connect
                Ping ping = new Ping();
                PingReply reply = ping.Send(server);

                if (reply.Status != IPStatus.Success)
                {
                    throw new CouldNotConnectException("The move.me server could not be connected to. Ping failed.");
                }
                else
                {
                    //Connect to the Playstation Move.Me server via TCP
                    try
                    {
                    ...
                    {
                        throw new CouldNotConnectException("The move.me server could not be connected to. " + e.Message);
                    }

                    //Create a UdpClient with an automatically assigned port number
                    _udpClient = new UdpClient(0);
                    Debug.WriteLine("PsMoveSharp Message: Initial recieve buffer size: {0}", _udpClient.Client.ReceiveBufferSize);

                    _udpClient.Client.ReceiveBufferSize = 655360; // 640 KB
                    Debug.WriteLine("PsMoveSharp Message: Expanded recieve buffer size: {0}", _udpClient.Client.ReceiveBufferSize);
            ...
            public void TrackAllHues()
            {
                ...
                SendRequestPacket(ClientRequest.PSMoveClientRequestTrackHues, trackHues, trackHues, trackHues, trackHues);
            }

            public void ChangeTrackHues(bool PickForMe)
            {
                if (PickForMe)
                    trackHues = (4 << 24);
                else
        ...
        public class PSMoveClientThreadedRead : PSMoveClient
        {
            ...
            protected void ReceiveCallback(IAsyncResult ar)
            {
                ...
                {
                    byte[] jpeg_data = new byte[packet_length - 3];
                    ...
                    Array.Copy(buffer, 23, jpeg_data, 0, packet_length - 3);
            ...
            protected void PSMoveClientThreadedReadThreadStart()
            {
                ...
                while (_readerThreadExit == 0)
                {
                    ...
                    catch (System.Exception)
                    {
                        ...
                        Debug.WriteLine("PsMoveSharp Message: except from udp client begin receive");
                    }
                    while (_reading)
                    {
            ...
            public uint GetLatestStateIndex()
            {
                _rwl.AcquireReaderLock(-1);
                uint index = _latest_state.packet_index;
                _rwl.ReleaseReaderLock();
                return index;
            }
        ...
        public class CouldNotConnectException : Exception, ISerializable
        {
            public CouldNotConnectException() : base() { }
            public CouldNotConnectException(string message) : base(message) { }
            public CouldNotConnectException(string message, System.Exception inner) : base(message, inner) { }
